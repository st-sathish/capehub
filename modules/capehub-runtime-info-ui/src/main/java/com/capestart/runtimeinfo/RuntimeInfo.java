package com.capestart.runtimeinfo;

import static com.capestart.rest.RestConstants.SERVICES_FILTER;
...
import com.capestart.security.api.Role;
...
import com.capestart.util.UrlSupport;
...
public class RuntimeInfo {
  ...
  public String getRuntimeInfo(@Context HttpServletRequest request) throws MalformedURLException {
    ...
    if (StringUtils.isNotBlank(orgEngageBaseUrl)) {
      ...
      logger.debug(
					"Using 'com.capestart.server.url' as a fallback for the non-existing organization level key '{}' for the components.json response",
					USER_URL_PROPERTY);
  ...
  protected JSONArray getRestEndpointsAsJson(HttpServletRequest request) throws MalformedURLException {
    ...
    for (ServiceReference servletRef : sort(serviceRefs)) {
      ...
      endpoint.put("docs", UrlSupport.concat(url.toExternalForm(), "/docs")); // This is a Capehub convention
